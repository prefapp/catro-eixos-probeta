...
module.exports = class {
		...
		Object.keys(parches).forEach((proceso) => {
			...
			parcheDatos.forEach((parche) => {
				...
				switch(parche.tipo){
                    ...
                    case "SIMULADOR_MPROCESO":

                        this.__parchesModulos[proceso] = new Parches["simulador_mproceso"](
	...
	test(fn){
        ...
        if( this.__hayParchesModulo ){

            this.__aplicarParchesModulos()
        }
	...
	__aplicarParcheInstanciacion(){
        ...
        const invocador = this.refProcesador.invocadorModulos;

		this.fOriginalProcesador = f;
        this.invocadorOriginal
        ...
        this.refProcesador.invocadorModulos = (modulo_llamada, args) => {

            if( this.__parchesModulos[modulo_llamada] ){

                return this.__parchesModulos[modulo_llamada].ejecutar(modulo_llamada, args)
            }
            else{

                return invocador(modulo_llamada, args)
	...
	__limpiarEjecucion(){
        ...
        this.refProcesador.invocador = this.invocadorOriginal
